...
.directive('initTablaarea', ['$compile', function($compile) {
 ...
 .controller('ControladorArea', ['$scope','$window','TareasResource','$location', function($scope,$window,TareasResource,$location) {
         ...
         var select= {
        Accion:'S',
        SQL:'SELECT A.ARE_CODI,A.ARE_NOMB,GA.GAR_NOMB ' + 
     ...
     var area =TareasResource.SQL(select);
        area.then(function(result){

            $('#tablearea').bootstrapTable('load', result.data);
