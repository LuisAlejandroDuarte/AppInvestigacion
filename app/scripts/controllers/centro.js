...
.directive('initTablacentro', ['$compile', function($compile) {
 ...
 .controller('ControladorCentro', ['$scope','$window','TareasResource','$location', function($scope,$window,TareasResource, $location) {
        ...
        var select = TareasResource.SQL({Accion:"S",SQL:"SELECT C.CEN_CODI,C.CEN_NOMB,Z.ZON_NOMB " + 
                    " FROM sgi_cent AS C RIGHT JOIN sgi_zona AS Z ON " +
                    " Z.ZON_CODI = C.CEN_ZONA_CODI"});

        select.then(function(result){

            $('#tablecentro').bootstrapTable('load',result.data);
